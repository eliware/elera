...
test('records recovery evidence, winner, state, and durable decision', async () => {
  const recoveryAudit = { evidence: jest.fn(), winner: jest.fn() }; const recoveryState = { set: jest.fn() }; const store = { write: jest.fn() }; const decision = { mode: 'bootstrap', reason: 'winner', epoch: 3, winner: 'node-a', evidence: [{ node: 'node-a' }] };
  ...
  expect(recoveryAudit.evidence).toHaveBeenCalledWith({ nodes: 1, mode: 'bootstrap' }); expect(recoveryAudit.winner).toHaveBeenCalledWith({ winner: 'node-a', epoch: 3 }); expect(recoveryState.set).toHaveBeenCalledWith('awaiting-quorum', { reason: 'winner', epoch: 3 }); expect(store.write).toHaveBeenCalledWith(decision);
});
