...
test('records a failed joined Primary validation', async () => {
  const recoveryAudit = { joinComplete: jest.fn(), failure: jest.fn() };
  await expect(verifySupervisorJoin({ elera: true, mode: 'join', sqlReady: true, health: { status: async () => ({ values: {} }) }, startupDecision: { epoch: 4 }, expectedSize: 2, recoveryState: { set: jest.fn() }, recoveryAudit, node: 'node-a.example.test' })).resolves.toBe(false);
...
test('accepts a joined member before the final cluster size is present', async () => {
  ...
  await expect(verifySupervisorJoin({ elera: true, mode: 'join', sqlReady: true, health: { status: async () => ({ values: { wsrep_cluster_state_uuid: 'cluster', wsrep_local_state_comment: 'Synced', wsrep_ready: 'ON', wsrep_cluster_status: 'Primary', wsrep_cluster_size: '2' } }) }, startupDecision: { epoch: 4 }, expectedSize: 3, recoveryState, recoveryAudit, node: 'node-a.example.test' })).resolves.toBe(true);
});
