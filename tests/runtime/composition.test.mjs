...
test('composes metadata and durable observation services from injected dependencies', () => {
  const result = createSupervisorComposition({ environment: { MARIADB_DATA_DIR: '/data', ELERA_OBSERVATION_STATE_PATH: '/run/observations.json', ELERA_CREDENTIAL_KEY: 'key' }, log: {}, query: async () => [[], []] });
...
test('uses in-memory observations and optional query dependency by default', () => {
  const result = createSupervisorComposition({ environment: {}, log: {}, query: async () => [[], []] });
