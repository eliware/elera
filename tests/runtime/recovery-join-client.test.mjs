...
test('posts an authenticated recovery join request and returns its data', async () => {
  ...
  await expect(client.join({ url: 'http://node-b.example.test/', epoch: 4, clusterId: 'cluster-a' })).resolves.toEqual({ status: 'joining' });
  expect(fetchImpl).toHaveBeenCalledWith('http://node-b.example.test/api/v1/cluster/cold-recovery/join', expect.objectContaining({ method: 'POST', body: JSON.stringify({ epoch: 4, clusterId: 'cluster-a' }), headers: expect.objectContaining({ authorization: 'Bearer secret' }) }));
});
...
test('reports structured remote join failures', async () => {
  ...
  await expect(client.join({ url: 'http://node-b.example.test', epoch: 4 })).rejects.toMatchObject({ message: 'UUID mismatch', code: 'JOIN_UUID_MISMATCH', statusCode: 409 });
});
...
test('handles non-JSON and unstructured remote failures', async () => {
  ...
  await expect(client.join({ url: 'http://node-b.example.test' })).rejects.toMatchObject({ message: 'recovery join returned 503', code: 'RECOVERY_JOIN_FAILED', statusCode: 503 });
});
