...
test('authorizes winner without mutating Galera state', async () => { const value = base(async () => ({ ok: true, json: async () => ({ data: { granted: true } }) })); const promote = jest.fn(); await expect(authorizeSupervisorRecovery({ ...value, promote })).resolves.toMatchObject({ args: ['--x', '--bootstrap'] }); expect(promote).not.toHaveBeenCalled(); expect(value.recoveryProtocol.authorize).toHaveBeenCalled(); expect(value.recoveryState.set).toHaveBeenCalledWith('recovery-authorized', expect.any(Object)); });
test('promotes an existing winner state file after recovery authorization', async () => { const value = base(async () => ({ ok: true, json: async () => ({ data: { granted: true } }) })); const promote = jest.fn(); const pathExists = jest.fn().mockResolvedValue(undefined); await expect(authorizeSupervisorRecovery({ ...value, promote, pathExists })).resolves.toMatchObject({ args: ['--x', '--bootstrap'] }); expect(pathExists).toHaveBeenCalledWith('/data/grastate.dat'); expect(promote).toHaveBeenCalledWith('/data/grastate.dat'); });
test('fails closed when winner state preparation fails', async () => { const value = base(async () => ({ ok: true, json: async () => ({ data: { granted: true } }) })); const error = new Error('state is unreadable'); await expect(authorizeSupervisorRecovery({ ...value, pathExists: jest.fn().mockRejectedValue(error), promote: jest.fn() })).rejects.toBe(error); });
