...
const base = () => ({ request: { winnerAddress: 'node-a', epoch: 4, clusterId: 'cluster-a', quorum: ['node-a', 'node-b'] }, identity: { name: 'node-b' }, args: ['--wsrep-cluster-address=gcomm://node-a,node-c'], mariaProcess: { start: jest.fn(async () => {}) }, recoveryState: { set: jest.fn() }, recoveryAudit: { joinStart: jest.fn() } });
...
test('starts a recovery join with the authorized winner address', async () => {
  ...
  expect(value.mariaProcess.start).toHaveBeenCalledWith(['--wsrep-cluster-address=gcomm://node-a']);
  ...
  expect(result).toMatchObject({ node: 'node-b', status: 'ready', epoch: 4 });
});
