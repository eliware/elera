...
describe('intent rendering and state', () => {
  ...
  test('cleans up after an initial apply write failure', async () => { const dir = await mkdtemp(join(tmpdir(), 'elera-intent-')); const state = createIntentState({ stateDir: dir }); await mkdir(state.paths.renderedPath); await expect(state.apply(fixture)).rejects.toBeTruthy(); });
});
