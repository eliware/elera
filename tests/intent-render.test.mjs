...
describe('intent rendering and state', () => {
  test('renders standalone and Galera configuration', () => { const standalone = structuredClone(fixture); delete standalone.mariadb.dataDir; expect(renderMariaDbConfig(standalone)).toContain('datadir=/var/lib/mysql'); const cluster = structuredClone(fixture); cluster.cluster.members.push({ name: 'galera-2', address: 'galera-2' }); expect(renderMariaDbConfig(cluster)).toContain('wsrep_on=ON'); });
  ...
  test('restores active state when rendered-file write fails', async () => { const dir = await mkdtemp(join(tmpdir(), 'galera-intent-')); const state = createIntentState({ stateDir: dir }); await state.apply(fixture); await unlink(state.paths.renderedPath); await mkdir(state.paths.renderedPath); const changed = structuredClone(fixture); changed.mariadb.port = 3307; await expect(state.apply(changed)).rejects.toBeTruthy(); expect(JSON.parse(await readFile(state.paths.activePath, 'utf8')).mariadb.port).toBe(3306); });
  test('propagates unexpected active-state read errors', async () => { const dir = await mkdtemp(join(tmpdir(), 'galera-intent-')); const state = createIntentState({ stateDir: dir }); await mkdir(state.paths.activePath); await expect(state.plan(fixture)).rejects.toBeTruthy(); });
});
