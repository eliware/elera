...
describe('intent rendering and state', () => {
  ...
  test('restores active state when rendered-file write fails', async () => { const dir = await mkdtemp(join(tmpdir(), 'galera-intent-')); const state = createIntentState({ stateDir: dir }); await state.apply(fixture); await mkdir(state.paths.renderedPath); const changed = structuredClone(fixture); changed.mariadb.port = 3307; await expect(state.apply(changed)).rejects.toBeTruthy(); expect(JSON.parse(await readFile(state.paths.activePath, 'utf8')).mariadb.port).toBe(3306); });
});
