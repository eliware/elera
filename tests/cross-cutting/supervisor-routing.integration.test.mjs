...
test('three supervisors converge on a writer while preserving reader candidates', async () => {
  const shared = new Map(); const nodes = ['elera-0', 'elera-1', 'elera-2']; const observations = nodes.map((node) => observation(node));
...
test('quorum loss stops assignment changes and conflicting cluster views are rejected', async () => {
  const observations = [observation('elera-0'), observation('elera-1'), observation('elera-2')];
  ...
  const set = jest.fn(); const coordinator = createQuorumAssignmentCoordinator({ assignmentStore: { get: async () => 'elera-0.elera', set }, observationStore: store, environment: { ELERA_CLUSTER_SIZE: '3' }, now: () => 100 });
  await expect(coordinator.write('payments', 'elera-0.elera')).rejects.toMatchObject({ code: 'QUORUM_REQUIRED' }); expect(set).not.toHaveBeenCalled();
...
test('rejects a split-brain view with conflicting primary identities', () => {
  const observations = [
    observation('elera-0'),
    observation('elera-1'),
    observation('elera-2', { primary: 'other-primary' })
  ];
...
test('last surviving member cannot claim a writer after total cluster loss', async () => {
  const observations = [observation('elera-0', { observedAt: 100 }), observation('elera-1', { observedAt: 0 }), observation('elera-2', { observedAt: 0 })];
  const store = createObservationStore({ now: () => 100 }); store.upsert(observations[0]);
  const coordinator = createQuorumAssignmentCoordinator({ assignmentStore: { get: async () => 'elera-0.elera', set: jest.fn() }, observationStore: store, environment: { ELERA_CLUSTER_SIZE: '3' }, now: () => 100 });
  await expect(coordinator.write('payments', 'elera-0.elera')).rejects.toMatchObject({ code: 'QUORUM_REQUIRED' });
  expect(selectCandidate([{ node: 'elera-0', uuid: 'cluster', seqno: 9 }, { node: 'elera-1', uuid: 'cluster', seqno: 8 }, { node: 'elera-2', uuid: 'cluster', seqno: 7 }])).toMatchObject({ eligible: true, candidate: { node: 'elera-0' } });
});
