...
const context = (method, path, body = {}, overrides = {}) => ({ method, path, url: new URL(path, 'http://localhost'), request: request(body), response: response(), getStatus: async () => ({ ready: true }), environment: { ELERA: '1', ELERA_NODE_NAME: 'node' }, ...overrides });
...
test('handles bootstrap confirmation, disabled mode, and unavailable lifecycle', async () => {
  ...
  await expect(handleClusterRoute(context('POST', '/api/v1/cluster/bootstrap', { confirm: true }, { environment: { ELERA: '0' }, bootstrap: jest.fn() }))).rejects.toMatchObject({ statusCode: 409 });
...
test('handles failed readiness polling and non-eligible bootstrap states', async () => {
  ...
  await expect(handleClusterRoute(context('GET', '/api/v1/cluster/bootstrap/eligibility', {}, { environment: { ELERA: '0' }, getStatus: async () => ({ ready: true }) }))).resolves.toBe(true);
});
