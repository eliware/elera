...
test('coordinates local and remote evidence and invokes the selected bootstrapper', async () => {
  ...
  const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'one.example.test', local: true }, { name: 'two.example.test', url: 'http://two.example.test' }], local, remote, bootstrapLocal, bootstrapRemote });
  const plan = await coordinator.plan();
  expect(plan).toBeDefined(); expect(local).toHaveBeenCalled(); expect(remote).toHaveBeenCalledWith('http://two.example.test', 'two.example.test');
});
...
test('executes the selected local candidate and revalidates its evidence', async () => {
  ...
  const bootstrapLocal = jest.fn(async () => {});
  const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'one.example.test', local: true }], local, remote, bootstrapLocal });
  await expect(coordinator.execute({ confirm: true, idempotencyKey: 'local-1' })).resolves.toMatchObject({ eligible: true, candidate: { node: 'one.example.test', uuid: 'u', seqno: 10 } });
  expect(bootstrapLocal).toHaveBeenCalledTimes(1);
  expect(await coordinator.execute({ confirm: true, idempotencyKey: 'local-1' })).toMatchObject({ candidate: { node: 'one.example.test' } });
});

test('uses the remote bootstrapper for a remote candidate', async () => {
  const remote = jest.fn(async (url) => url === 'http://two.example.test' ? { state: { uuid: 'u', seqno: 10, safeToBootstrap: true }, active: false } : { state: { uuid: 'u', seqno: 9, safeToBootstrap: false }, active: false });
  const bootstrapRemote = jest.fn(async () => {});
  const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'one.example.test', local: true }, { name: 'two.example.test', url: 'http://two.example.test' }], local: remote, remote, bootstrapLocal: jest.fn(), bootstrapRemote });
  await expect(coordinator.execute({ confirm: true })).resolves.toMatchObject({ candidate: { node: 'two.example.test' } });
  expect(bootstrapRemote).toHaveBeenCalledWith(expect.objectContaining({ name: 'two.example.test', url: 'http://two.example.test' }));
});
...
test('refuses remote execution when no remote bootstrapper is configured', async () => {
  ...
  const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'remote.example.test', url: 'http://remote.example.test' }], local: evidence, remote: evidence, bootstrapLocal: jest.fn() });
...
test('recovers a missing sequence number through the local evidence reader', async () => {
  ...
  const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'one.example.test', local: true, dataDir: '/data' }], local, remote: jest.fn(), bootstrapLocal: jest.fn() });
...
test('persists an idempotent local bootstrap result when a lock path is configured', async () => {
  ...
  try {
    ...
    const coordinator = createColdBootstrapCoordinator({ nodes: [{ name: 'one.example.test', local: true }], local, remote: jest.fn(), bootstrapLocal, lockPath: join(directory, 'lock') });
...
test('dispatches a verified local candidate to the local bootstrap callback', async () => {
  ...
  const coordinator = createColdBootstrapCoordinator({
    nodes: [{ name: 'local.example.test', local: true }],
...
test('dispatches a verified remote candidate to the remote bootstrap callback', async () => {
  ...
  const coordinator = createColdBootstrapCoordinator({
    nodes: [{ name: 'remote.example.test', url: 'http://remote.example.test' }],
  ...
  await coordinator.execute({ confirm: true });
  expect(bootstrapRemote).toHaveBeenCalledWith({ name: 'remote.example.test', url: 'http://remote.example.test' });
});
