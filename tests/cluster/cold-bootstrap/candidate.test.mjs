...
test('deterministically resolves equivalent ties and rejects UUID mismatch', () => { expect(selectCandidate([node('b', 10), node('a', 10)])).toMatchObject({ eligible: true, candidate: { node: 'a' } }); expect(selectCandidate([node('a', 10), { ...node('b', 9), uuid: 'other' }]).reason).toContain('UUID'); });
