...
const nodeName = 'a.example.test';

test('reads persisted state and recovers an unavailable sequence number', async () => {
  const evidence = createStartupLocalEvidence({ node: { name: nodeName }, dataDir: 'x', readState: async () => ({ uuid: 'c', seqno: -1, safeToBootstrap: false }), runRecover: async () => 'Recovered position: c:42' });
  await expect(evidence()).resolves.toMatchObject({ node: nodeName, state: { uuid: 'c', seqno: 42 } });
});
...
test('does not recover while the local database process is active', async () => {
  ...
  const evidence = createStartupLocalEvidence({ node: { name: nodeName }, dataDir: 'x', readState: async () => ({ uuid: 'c', seqno: -1, safeToBootstrap: false }), runRecover, isActive: () => true });
  await expect(evidence()).resolves.toMatchObject({ node: nodeName, active: true, state: { seqno: -1 } });
});
...
test('fails closed when recovery cannot report a sequence number', async () => {
  const evidence = createStartupLocalEvidence({ node: { name: nodeName }, dataDir: 'x', readState: async () => ({ uuid: 'c', seqno: -1, safeToBootstrap: false }), runRecover: async () => 'no recovered position' });
...
test('does not rerun a failed wsrep recovery probe for every evidence request', async () => {
  let calls = 0;
  const evidence = createStartupLocalEvidence({ node: { name: nodeName }, dataDir: 'x', readState: async () => ({ uuid: 'c', seqno: -1, safeToBootstrap: false }), runRecover: async () => { calls += 1; throw new Error('recover unavailable'); } });
...
test('shares one in-flight wsrep recovery probe across concurrent evidence requests', async () => {
  let calls = 0;
  let release;
  const evidence = createStartupLocalEvidence({ node: { name: nodeName }, dataDir: 'x', readState: async () => ({ uuid: 'c', seqno: -1, safeToBootstrap: false }), runRecover: async () => { calls += 1; await new Promise((resolve) => { release = resolve; }); return 'Recovered position: c:42'; } });
