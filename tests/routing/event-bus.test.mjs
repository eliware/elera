...
test('replays the latest event when a subscriber filter accepts it', () => { bus = createRoutingEventBus(); const send = jest.fn(); bus.publish({ type: 'routing.update' }); bus.subscribe({ send }, () => true); expect(send).toHaveBeenCalledWith({ type: 'routing.update' }); });
