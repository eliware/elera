...
describe('HTTP probes', () => {
  ...
  test('draining nodes are not ready for HAProxy', async () => { await new Promise((resolve) => server.close(resolve)); server = createProbeServer({ getStatus: async () => ({ ready: true }), isDraining: () => true, controlHandler: async (_request, response) => response.writeHead(202).end('control'), log: { warn: jest.fn() } }); const port = await listen(server); expect((await get(port, '/healthz')).status).toBe(200); expect((await get(port, '/readyz')).status).toBe(503); });
  ...
  test('rejects new control requests after shutdown begins while keeping health live', async () => { await new Promise((resolve) => server.close(resolve)); server = createProbeServer({ getStatus: async () => ({ ready: true }), isShuttingDown: () => true, controlHandler: async () => {}, log: { warn: jest.fn() } }); const port = await listen(server); expect((await get(port, '/healthz')).status).toBe(200); expect((await get(port, '/api/v1/status')).status).toBe(503); });
