...
describe('intent reconciliation', () => {
  ...
  test('rejects membership changes as unsafe', async () => { const desired = structuredClone(fixture); desired.cluster.members.push({ name: 'two.cluster.local', address: 'two.cluster.local' }); await expect(reconcileIntent({ desired, active: fixture, apply: async () => {} })).rejects.toMatchObject({ code: 'UNSAFE_INTENT_CHANGE', statusCode: 409 }); });
