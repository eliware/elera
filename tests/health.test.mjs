...
const values = { wsrep_local_state_comment: 'Synced', wsrep_ready: 'ON', wsrep_cluster_status: 'Primary', wsrep_local_recv_queue: '0', wsrep_local_send_queue: '0', wsrep_flow_control_paused: '0' };
describe('health service', () => {
  test('caches concurrent and subsequent checks', async () => { const query = jest.fn(async () => [[...Object.entries(values).map(([Variable_name, Value]) => ({ Variable_name, Value }))]]); const service = createHealthService({ db: { query }, timeoutMs: 100, log: { debug: jest.fn() } }); expect((await Promise.all([service.status(), service.status()]))[0].ready).toBe(true); await service.status(); expect(query).toHaveBeenCalledTimes(1); expect(service.cacheInfo().cached).toBe(true); });
