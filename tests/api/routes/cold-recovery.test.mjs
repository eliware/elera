...
test('covers every authenticated recovery operation and protocol absence', async () => {
  ...
  const initializingPlan = response(); await expect(handleColdRecoveryRoute({ method: 'POST', path: '/api/v1/cluster/cold-recovery/plan', response: initializingPlan, protocol: undefined, auth })).resolves.toBe(true); expect(initializingPlan.json).toHaveBeenCalledWith(200, expect.objectContaining({ data: expect.objectContaining({ eligible: false, mode: 'blocked' }) }));
  await expect(handleColdRecoveryRoute({ method: 'POST', path: '/api/v1/cluster/cold-recovery/plan', response: response(), protocol: undefined, auth: {} })).resolves.toBe(false);
