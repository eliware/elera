...
describe('supervisor configuration', () => {
  ...
  test('leaves Galera provider recovery enabled for ordinary restarts', () => { const intent = { apiVersion: 'elera.eliware.dev/v1alpha1', cluster: { name: 'c', members: [{ name: 'n', address: 'n' }, { name: 'p', address: 'p' }] }, mariadb: { port: 3306 }, routing: { healthIntervalMs: 1000 }, drain: { queryTimeoutMs: 1 } }; expect(mariaDbArguments(loadSupervisorConfig({}, intent))).toContain('--wsrep-provider-options=pc.recovery=TRUE'); });
