...
const base = { node: 'node-a.example.test', expectedPath: 'C:\\data', request: { node: 'node-a.example.test', dataDir: 'C:\\data', confirmation: 'RESET node-a.example.test', dryRun: true }, getStatus: async () => ({ values: {} }) };
...
test('rejects unsafe or malformed requests', async () => {
  await expect(validateNodeDataReset({ ...base, request: { ...base.request, node: 'node-b.example.test' } })).rejects.toThrow('identity');
...
test('covers executing, default, and status edge conditions', async () => {
  await expect(validateNodeDataReset({ ...base, request: { node: 'node-a.example.test', dataDir: 'C:\\data', confirmation: 'RESET node-a.example.test', idempotencyKey: 'key' }, getRecoveryState: () => null })).resolves.toMatchObject({ dryRun: false, key: 'key' });
