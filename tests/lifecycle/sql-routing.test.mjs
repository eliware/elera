...
test('updates primary and balanced client availability during drain', () => {
  ...
  const integration = createSqlDrainIntegration({ getClient: () => client, node: 'node-1' });
  integration(true); integration(false);
  expect(client.setNodeAvailability).toHaveBeenNthCalledWith(1, 'primary', 'node-1', false);
  expect(client.setNodeAvailability).toHaveBeenNthCalledWith(4, 'balanced', 'node-1', true);
});
