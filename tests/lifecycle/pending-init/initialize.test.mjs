...
test("initialization validates, starts private MariaDB, applies SQL, and stops it", async () => {
  ...
  expect(calls.some((value) => value?.sql?.includes("CREATE DATABASE IF NOT EXISTS `elera_meta`"))).toBe(true);
...
test("initialization reports a MariaDB readiness timeout", async () => {
  ...
  await expect(initializePendingData({ environment: {}, run: async (command) => { if (command === "mariadb-admin") throw new Error("not ready"); }, start, sleep: async () => {} })).rejects.toThrow("not ready");
});
...
test("initialization retries a transient MariaDB readiness failure", async () => {
  ...
  await initializePendingData({ environment: {}, run: async (command) => {
