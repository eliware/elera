...
describe('supervisor intent model', () => {
  ...
  test('loads defaults and rejects malformed JSON or intent', () => { expect(defaultIntent({}).cluster.name).toBe('local-galera'); expect(loadIntent({}).mariadb.port).toBe(3306); expect(() => loadIntent({ SUPERVISOR_INTENT_JSON: '{' })).toThrow('invalid SUPERVISOR_INTENT_JSON'); expect(() => loadIntent({ SUPERVISOR_INTENT_JSON: '{}' })).toThrow('invalid supervisor intent'); });
