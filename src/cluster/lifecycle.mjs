...
export function createLifecycleManager({ status, operations = {}, environment = process.env, config = {} }) {
  if (typeof status !== 'function') throw new TypeError('status function is required');
  return {
    async plan(action, target) { const current = await status(); return planLifecycle(action, { enabled: config.elera ?? config.clusterSize > 1, ready: current.ready, synced: current.values?.wsrep_local_state_comment === 'Synced', quorum: current.values?.wsrep_cluster_status === 'Primary', nodeId: config.runtimeNodeName ?? 'elera', target }); },
