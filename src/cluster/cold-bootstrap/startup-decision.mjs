...
export function createStartupRecoveryDecision({ nodes, expectedNodeCount = nodes?.length, localEvidence, fetchEvidence, attempts = 15, delayMs = 1000, epoch = (evidence) => createHash('sha256').update(JSON.stringify(evidence.toSorted((a, b) => a.node.localeCompare(b.node)))).digest('hex') } = {}) {
  ...
  return async function decide() {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        return { mode: 'bootstrap', winner: selected, localWinner: selected === nodes.find((node) => node.local)?.name, reason: decision.reason, epoch: epoch(states), evidence: states };
