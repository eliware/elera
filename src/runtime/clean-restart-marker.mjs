...
export function createCleanRestartMarker({ path, node, epoch, nonce = randomUUID(), now = Date.now, maxAgeMs = 120000 } = {}) {
  ...
  return {
    ...
    async consume({ expectedNode = node, expectedEpoch = epoch } = {}) {
      let value;
      try { value = JSON.parse(await readFile(path, 'utf8')); } catch { return undefined; }
      try { await unlink(path); } catch {}
      if (value?.version !== 1 || value.node !== expectedNode || value.nonce !== nonce || value.epoch !== (expectedEpoch ?? null) || !Number.isFinite(value.writtenAt) || now() - value.writtenAt < 0 || now() - value.writtenAt > maxAgeMs) return undefined;
      return value;
    },
  };
}
