...
import { handleNodeDataResetRoute } from './api/routes/node-data-reset.mjs';

export function createControlApi({ db, getStatus, getTraffic, getTelemetry, getTelemetryDetails, setDrain, bootstrap, coldBootstrap, getColdBootstrap, coldEvidence, getColdEvidence, coldBootstrapLocal, getColdBootstrapLocal, getColdRecoveryProtocol, lifecycle, getConfig, getActiveIntent, leaseCredentials, routingBundles, routingEvent, recovery, metadata, managed, applications, reconciler, artifactStore, observations = [], observationStore, nodeDataReset, identity, environment = process.env, log, dataDir = environment.MARIADB_DATA_DIR ?? '/var/lib/mysql' }) {
  if (!identity?.name) throw new TypeError('shared runtime identity is required for the control API');
