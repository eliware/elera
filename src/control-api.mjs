...
export function createControlApi({ db, getStatus, getTraffic, getTelemetry, getTelemetryDetails, setDrain, bootstrap, coldBootstrap, getColdBootstrap, coldEvidence, getColdEvidence, coldBootstrapLocal, getColdBootstrapLocal, getColdRecoveryProtocol, lifecycle, getConfig, getActiveIntent, leaseCredentials, routingBundles, routingEvent, recovery, metadata, managed, applications, reconciler, artifactStore, observations = [], observationStore, nodeDataReset, environment = process.env, log, dataDir = environment.MARIADB_DATA_DIR ?? '/var/lib/mysql' }) {
