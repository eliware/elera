...
export async function handleIntentRoute({ method, path, request, response, environment, identity, getActiveIntent }) {
  if (!identity?.name) throw new Error('runtime identity is required for intent routes');
  if (method === 'GET' && path === '/api/v1/config/intent') { const intent = loadIntent(environment, identity); response.json(200, { ok: true, operation: 'config.intent', status: 'completed', data: { intent, desiredHash: intentHash(intent) } }); return true; }
  ...
  if (method === 'POST' && path === '/api/v1/config/verify') { if (!getActiveIntent?.verify) throw Object.assign(new Error('config state is not configured'), { statusCode: 503 }); response.json(200, { ok: true, operation: 'config.verify', status: 'completed', data: await getActiveIntent.verify(loadIntent(environment, identity)) }); return true; }
  return false;
}
