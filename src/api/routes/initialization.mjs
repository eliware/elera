...
export async function handleInitializationRoute({ method, path, request, response, db, environment, dataDir }) {
  ...
  if (method === 'POST' && path === '/api/v1/initialization/plan') { response.json(200, { ok: true, operation: 'initialization.plan', changed: false, status: 'planned', data: { elera: environment.ELERA === '1', database: environment.MARIADB_DATABASE ?? null, user: environment.MARIADB_USER ?? null } }); return true; }
