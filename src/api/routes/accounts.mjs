...
export async function handleAccountRoute({ method, path, request, response, db }) {
  ...
  if (method === 'GET' && path === '/api/v1/accounts') { const [rows] = await db.query("SELECT User, Host, plugin FROM mysql.user WHERE User NOT IN ('mariadb.sys','mysql','root') ORDER BY User, Host"); send(response, 200, { ok: true, accounts: rows }); return true; }
