...
export async function handleObservationRoute({ method, path, request, response, environment, getStatus, observations, observationStore }) {
  ...
  if (method === 'GET' && path === '/api/v1/cluster/topology') { response.json(200, { ok: true, operation: 'cluster.topology', status: 'completed', data: { observations: store.snapshot(), quorum: evaluateQuorum(store.snapshot()) } }); return true; }
