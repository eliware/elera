...
export function createPendingInitServer({ environment = process.env, log = console, initialize = initializePendingData, onInitialized = () => {} } = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && ["/api/v1/cluster/bootstrap", "/api/v1/cluster/join", "/api/v1/initialization/apply"].includes(request.url)) {
      const operationName = request.url.endsWith("/join") ? "join" : request.url.endsWith("/initialization/apply") ? "standalone-init" : "bootstrap";
      ...
      try {
        await operation;
        json(response, 202, { ok: true, operation: operationName === "standalone-init" ? "initialization.apply" : `cluster.${operationName}`, status: "completed" });
